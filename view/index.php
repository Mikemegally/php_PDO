<?php
    session_start();
    if(isset($_COOKIE['active_user'])){
        header("location:./home_page.php");
        exit();
    }
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>section2</title>
    <link rel="stylesheet" href="../css/section3.css?v=<?php echo time() ?>">
</head>
<body>
    <div class="conatiner">
        <h1>registration form</h1>
        <form action="../backend/register_back.php" method="POST">
            <label for="username">username</label><br>
            <input type="text" id="username" name="name" placeholder="enter your username" required maxlength="50"><br>

            <label for="email">email</label><br>
            <input type="email" id="email" name="email" placeholder="enter your mail" required maxlength="50"><br>

            <label for="password">password</label><br>
            <input type="password" id="password" name="pas" placeholder="enter your password" required maxlength="50"><br>

            <label for="confirm">confirm password</label><br>
            <input type="password" id="confirm" name="cu" placeholder="enter your password again"><br><br>
            <p style="color:red;font-weight:bold;">
                <?php
                    if(isset($_SESSION['error'])){
                        echo $_SESSION['error'];
                        unset($_SESSION['error']);
                    }
                ?>
            </p>
            <button type="submit" name="btn">register</button>
            <span> have an account? <a href="login.php">login instead</a> OR <a href="delete.php">DELETE YOUR ACOUNT</a> </span>
        </form>
    </div>
</body>
</html>